...
module.exports = class UsersDBApi {
  ...
  static async update(id, data, globalAccess, options) {
    ...
    await users.setCompanies(data.companies || null, {
      transaction,
    });
  ...
  static async findBy(where, options) {
    ...
    output.cvs_job_seeker = await users.getCvs_job_seeker({
      transaction,
    });

    output.job_offers_business_owner = await users.getJob_offers_business_owner(
      {
        transaction,
      },
    );

    output.job_offers_job_seeker = await users.getJob_offers_job_seeker({
      transaction,
    });

    output.job_postings_hr_manager = await users.getJob_postings_hr_manager({
      transaction,
    });
