...
const EditCvs = () => {
  ...
  const { cvsId } = router.query;

  useEffect(() => {
    dispatch(fetch({ id: cvsId }));
  }, [cvsId]);
  ...
  const handleSubmit = async (data) => {
    await dispatch(update({ id: cvsId, data }));
...
EditCvs.getLayout = function getLayout(page: ReactElement) {
  return (
    <LayoutAuthenticated permission={'UPDATE_CVS'}>{page}</LayoutAuthenticated>
  );
};

export default EditCvs;
